...
(function () {
    ...
    const DECISION_STORAGE_KEY = "br::wemc::decision";
    ...
    const conditions = {
        ...
        /**
         * Assigns a stable random result to this condition for the current
         * browser session. The opaque reference is generated by Script
         * Creator, so each configured random condition has its own result.
         */
        random: {
            evaluate: function (condition, runtime) {
                return _private.getRandomConditionResult(runtime, condition);
            }
        },
    ...
    const _private = {
        ...
        getBrowserStorage: function (scope) {
        ...
        readDecisionStorage: function (scope) {
            const storage = this.getBrowserStorage(scope);
            if (storage === null) {
                return null;
            }
        ...
        getRandomStorageEntryKey: function (runtime, refId) {
            return [
                String(runtime && runtime.webExId || ""),
                String(runtime && runtime.webExVersionId || ""),
                String(refId || "")
        ...
        readRandomStorage: function () {
            const storage = this.getBrowserStorage(DECISION_CACHE_SCOPE_SESSION);
            if (storage === null) {
                return null;
            }

            try {
                const raw = storage.getItem(RANDOM_STORAGE_KEY);
                const parsed = raw === null ? null : JSON.parse(raw);
                if (!$.isPlainObject(parsed) || parsed.version !== RANDOM_STORAGE_VERSION ||
                    !$.isPlainObject(parsed.entries)) {
                    return {
                        storage: storage,
                        entries: {}
        ...
        writeRandomStorage: function (storageData) {
            if (!storageData || !storageData.storage || !$.isPlainObject(storageData.entries)) {
                return;
            }

            try {
                storageData.storage.setItem(RANDOM_STORAGE_KEY, JSON.stringify({
                    version: RANDOM_STORAGE_VERSION,
                    entries: storageData.entries
                }));
            } catch (e) {
                // Storage may be disabled or full. The in-memory result remains usable.
            }
        },

        getRandomConditionReference: function (condition) {
            return $.isPlainObject(condition)
                ? Breinify.UTL.isNonEmptyString(condition.randomRefId)
                : null;
        },

        getRandomConditionResult: function (runtime, condition) {
            const refId = this.getRandomConditionReference(condition);
            const settings = this.getConditionSettings(condition);
            const probability = settings && settings.probability;
            if (refId === null || typeof probability !== "number" || !isFinite(probability) ||
                probability < 0 || probability > 1) {
                return false;
            }

            const results = runtime.randomConditionResults || {};
            if (typeof results[refId] === "boolean") {
                return results[refId];
            }

            const storageData = this.readRandomStorage();
            const storageKey = this.getRandomStorageEntryKey(runtime, refId);
            const stored = storageData === null ? null : storageData.entries[storageKey];
            const result = typeof stored === "boolean" ? stored : Math.random() < probability;
            results[refId] = result;
            runtime.randomConditionResults = results;

            if (storageData !== null && typeof stored !== "boolean") {
                storageData.entries[storageKey] = result;
                this.writeRandomStorage(storageData);
            }
