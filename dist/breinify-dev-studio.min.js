(()=>{let o=Breinify.UTL._jquery(),t={resizable:function(t){let i=o("body"),e=t.find("#resize-handle").data("isResizing",!1);e.mousedown(t=>{e.data({isResizing:!0,startX:t.clientX,startWidth:e.parent()[0].getBoundingClientRect().width}),i.css("user-select","none"),t.preventDefault()}),o(document).on("mouseup blur",()=>{!0===e.data("isResizing")&&(e.data("isResizing",!1),i.css("user-select",""))}),o(document).mousemove(t=>{var i;!0!=!e.data("isResizing")&&(i=e.data("startWidth"),t=e.data("startX")-t.clientX,i=Math.min(Math.max(i+t,200),1e3),e.parent().css("width",i+"px"))})}};class i extends HTMLElement{$shadowRoot=null;$toggleButton=null;$panel=null;$closeBtn=null;$tabs=null;$logContainer=null;$infoContainer=null;constructor(){super(),this.attachShadow({mode:"open"}),this.isVisible=!0,this.render(),this.toggleDevStudio()}render(){this.shadowRoot.innerHTML=`
            <style>
                :host { all: initial; }
            ...
            <div id="toggle-button" title="Show Breinify DevStudio" role="button" tabindex="0"><svg xmlns="http://www.w3.org/2000/svg" fill="white" width="16" height="16" viewBox="0 0 24 24"><path d="M12 2C8.1 2 6 4.4 6 7v5c0 .5-.2.9-.5 1.3-.3.4-.5.9-.5 1.4v.3c.1.6.5 1.1 1 1.5.5.4.8 1 .8 1.6 0 .6.2 1.1.5 1.5s.7.7 1.2.9V21c0 .6.4 1 1 1s1-.4 1-1v-1h2v1c0 .6.4 1 1 1s1-.4 1-1v-1.5c.5-.2.9-.5 1.2-.9s.5-.9.5-1.5c0-.6.3-1.2.8-1.6.5-.4.9-.9 1-1.5v-.3c0-.5-.2-1-.5-1.4-.3-.4-.5-.9-.5-1.3V7c0-2.6-2.1-5-6-5z"/></svg></div>`,this.$shadowRoot=o(this.shadowRoot),this.$toggleButton=this.$shadowRoot.find("#toggle-button"),this.$panel=this.$shadowRoot.find("#panel"),this.$closeBtn=this.$shadowRoot.find("button.close-btn"),this.$tabs=this.$shadowRoot.find("button.tab"),this.$logContainer=this.$shadowRoot.find("#log-container"),this.$infoContainer=this.$shadowRoot.find("#info-container"),this.$closeBtn.click(()=>this.toggleDevStudio()),this.$toggleButton.click(()=>this.toggleDevStudio()),this.$tabs.click(t=>this._switchTab(t)),t.resizable(this.$shadowRoot)}toggleDevStudio(){this.isVisible=!this.isVisible,this.isVisible?(this.$panel.css("transform","translateY(0)"),this.$panel.css("opacity","1"),this.$toggleButton.css("display","none")):(this.$panel.css("transform","translateY(100%)"),this.$panel.css("opacity","0"),this.$toggleButton.css("display","flex"))}_refreshInfo(){var t="string"==typeof Breinify.version&&""!==Breinify.version.trim()?Breinify.version:"Unknown",i=Object.keys(Breinify.plugins).filter(t=>"_"!==t.charAt(0)&&o.isPlainObject(Breinify.plugins[t])).sort(),t=(this.$infoContainer.empty(),this.$infoContainer.append(o('<div class="info-section"></div>').append(o('<div class="info-label">Script version</div>')).append(o('<div class="info-value"></div>').text(t))),o('<div class="info-section"></div>'));t.append(o('<div class="info-label"></div>').text("Loaded plugins ("+i.length+")"));let e=o('<ul class="plugin-list"></ul>');i.forEach(t=>{e.append(o("<li></li>").text(t))}),t.append(e),this.$infoContainer.append(t)}_switchTab(t){let i=t.target.dataset.tab;this.$tabs.each(function(){this.classList.toggle("active",this.dataset.tab===i)}),"console"===i?(this.$logContainer.addClass("active"),this.$infoContainer.removeClass("active")):"info"===i&&(this._refreshInfo(),this.$logContainer.removeClass("active"),this.$infoContainer.addClass("active"))}}let e=Breinify.plugins._add("devStudio",{devStudio:null,init:function(){var t;!0===Breinify.UTL.internal.isDevMode()&&null===this.devStudio&&(t="breinify-dev-console",customElements.define(t,i),this.devStudio=document.createElement(t),document.body.appendChild(this.devStudio))}});Breinify.onReady(function(){e.init()})})();