(()=>{class o extends HTMLElement{$shadowRoot=null;$toggleButton=null;$panel=null;$closeBtn=null;$tabs=null;$logContainer=null;$infoContainer=null;constructor(){super(),this.attachShadow({mode:"open"}),this.isVisible=!0,this.render(),this.hookConsole(),this.toggleDevStudio()}render(){this.shadowRoot.innerHTML=`
            <style>
                :host { all: initial; }
            ...
            <div id="toggle-button" title="Show Breinify DevStudio" role="button" tabindex="0"><svg xmlns="http://www.w3.org/2000/svg" fill="white" width="16" height="16" viewBox="0 0 24 24"><path d="M12 2C8.1 2 6 4.4 6 7v5c0 .5-.2.9-.5 1.3-.3.4-.5.9-.5 1.4v.3c.1.6.5 1.1 1 1.5.5.4.8 1 .8 1.6 0 .6.2 1.1.5 1.5s.7.7 1.2.9V21c0 .6.4 1 1 1s1-.4 1-1v-1h2v1c0 .6.4 1 1 1s1-.4 1-1v-1.5c.5-.2.9-.5 1.2-.9s.5-.9.5-1.5c0-.6.3-1.2.8-1.6.5-.4.9-.9 1-1.5v-.3c0-.5-.2-1-.5-1.4-.3-.4-.5-.9-.5-1.3V7c0-2.6-2.1-5-6-5z"/></svg></div>`,this.$shadowRoot=$(this.shadowRoot),this.$toggleButton=this.$shadowRoot.find("#toggle-button"),this.$panel=this.$shadowRoot.find("#panel"),this.$closeBtn=this.$shadowRoot.find("button.close-btn"),this.$tabs=this.$shadowRoot.find("button.tab"),this.$logContainer=this.$shadowRoot.find("#log-container"),this.$infoContainer=this.$shadowRoot.find("#info-container"),this.$closeBtn.click(()=>this.toggleDevStudio()),this.$toggleButton.click(()=>this.toggleDevStudio()),this.$tabs.click(t=>this.switchTab(t))}toggleDevStudio(){this.isVisible=!this.isVisible,this.isVisible?(this.$panel.css("transform","translateY(0)"),this.$panel.css("opacity","1"),this.$toggleButton.css("display","none")):(this.$panel.css("transform","translateY(100%)"),this.$panel.css("opacity","0"),this.$toggleButton.css("display","flex"))}switchTab(t){let o=t.target.dataset.tab;this.$tabs.each(function(){this.classList.toggle("active",this.dataset.tab===o)}),"console"===o?(this.$logContainer.addClass("active"),this.$infoContainer.removeClass("active")):"info"===o&&(this.$logContainer.removeClass("active"),this.$infoContainer.addClass("active"))}hookConsole(){["log","warn","error","info"].forEach(i=>{let n=console[i];console[i]=(...t)=>{var o=t.map(t=>{try{return"object"==typeof t&&null!==t?JSON.stringify(t,null,2):String(t)}catch{return"[object]"}}).join(" "),e=document.createElement("div");e.textContent=`[${i.toUpperCase()}] `+o,e.style.color={log:"white",warn:"orange",error:"red",info:"lightblue"}[i],this.$logContainer.append(e),this.$logContainer.get(0).scrollTop=this.$logContainer.get(0).scrollHeight,n.apply(console,t)}})}}let t=Breinify.plugins._add("devStudio",{devStudio:null,init:function(){var t;null===this.devStudio&&(t="breinify-dev-console",customElements.define(t,o),this.devStudio=document.createElement(t),document.body.appendChild(this.devStudio))}});Breinify.onReady(function(){t.init()})})();